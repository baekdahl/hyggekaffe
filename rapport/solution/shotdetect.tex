For understanding when a new state has occurred a method for detecting this has to be developed. A new state is, as written in section \ref{sec:sysdesign} when the balls have moved or changed in numbers. The method for detecting this will consist of three steps:
\begin{enumerate}
...
To understand when an occlusion is present several different test scenarios have been tested, which can be seen in table \ref{fig:occtab1} and  \ref{fig:occtab2}.

\subsubsection{Test Images With Table Mask, Area, Perimeter and Calculated Factors}
\begin{table}[H]
